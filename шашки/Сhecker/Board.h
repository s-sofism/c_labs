...
public:
	...
	void draw_board(RenderWindow &_window)//рисуем поле
	{
		for (int i = 1; i <= 8; i++) 
		{
			for (int j = 1; j <= 8; j++)
			{
				...
				if ((i + j) % 2 == 1) all_squares[i - 1][j - 1].set_color(Color(101, 61, 16));
				else all_squares[i - 1][j - 1].set_color(Color(255, 178, 115));
