...
public:
	...
	void start_game() 
	{
		for (int i = 0; i < 24; i++)//создаем шашки
		{
			if (i < 4)
			{
	...
	void draw_checkers(RenderWindow &_window) //рисуем поле с фигурами
	{
		board.draw_board(_window);
		for (int i = 0; i < checkers.size(); i++) 
		{
			checkers[i].draw_checker(_window);//рисуем каждую шашку
		}
	}

	void delete_backlight()//удаляем подсветку клеток
	{
		for (int i = 0; i < 8; i++) 
		{
			for (int j = 0; j < 8; j++) 
