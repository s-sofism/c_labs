...
private:
	int how_many = 0;
	bool who_can_move = 0;//0 если ход белых, 1 если ход черных
...
public:
	...
	Vector2i centre_on_square()//определяем центр квадрата
	{
		for (int i = 0; i < 8; i++) 
		{
			for (int j = 0; j < 8; j++)
	...
	void choise_of_chacker(RenderWindow &_window, Event _event)//выбираем шашку
	{
	...
	void change_position(RenderWindow &_window, Event _event)//меняем позицию шашки
	{
		if (_event.type == sf::Event::MouseButtonPressed) 
		{
	...
	void make_move(RenderWindow &_window, Event _event)//делаем сам ход
	{
		choise_of_chacker(_window, _event);
		change_position(_window, _event);
	}

	void square_for_move()//варианты ходов и подсветка
	{
	...
	bool eat_checker()//поедание шашки
	{
	...
	bool queen_eat_checker()//поедание шашки королевой
	{
		int x_eat = 0;
		int y_eat = 0;
	...
	bool end_board(float _x, float _y)//проверка выхода за пределы поля
	{
